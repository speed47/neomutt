...
/**
 * free_query - Free one query
 * @param[out] qp Query to free
 */
static void free_query(struct Query **qp)
{
  if (!qp || !*qp)
    return;
...
/**
 * free_query - Free all elements of a QueryVector an reinitialize it
 * @param[out] vector QueryVector to free
 */
static void free_queryvector(QueryVector *vector)
{
  struct Query *q;
  int iter;
